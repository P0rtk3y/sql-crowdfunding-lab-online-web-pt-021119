...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS remaining_amount FROM projects JOIN pledges on projects.id = pledges.project_id GROUP BY projects.title HAVING remaining_amout < 0;"
end
